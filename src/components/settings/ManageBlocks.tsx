...
import { Loader2, Plus, Trash2, Edit, Save, X, Building } from 'lucide-react';
...
const ManageBlocks: React.FC = () => {
  const queryClient = useQueryClient();
  const [newItemName, setNewItemName] = useState('');
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingName, setEditingName] = useState('');

  const configType = 'blocks';
  const itemName = 'Bloco';

  const { data: items, isLoading, error } = useQuery<BlockItem[], Error>({
    queryKey: [configType],
    queryFn: async () => {
  ...
  const handleEdit = (item: BlockItem) => {
  ...
  return (
    <Card className="backdrop-blur-sm bg-white/70 dark:bg-gray-800/70 shadow-xl border border-white/30 dark:border-gray-700/50">
      <CardHeader>
        <CardTitle className="flex items-center"><Building className="w-5 h-5 mr-2" /> Gerenciar Blocos</CardTitle>
      </CardHeader>
      <CardContent>
