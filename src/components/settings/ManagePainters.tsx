...
import { Loader2, Plus, Trash2, Edit, Save, X, PaintBucket } from 'lucide-react';
...
const fetchPainters = async (): Promise<PainterItem[]> => {
  const { data, error } = await supabase.from('painters').select('id, name').order('name');
  if (error) throw error;
  return data as PainterItem[];
};

const ManagePainters: React.FC = () => {
  const queryClient = useQueryClient();
  const [newItemName, setNewItemName] = useState('');
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingName, setEditingName] = useState('');

  const configType = 'painters';
  const itemName = 'Pintor';

  const { data: items, isLoading, error } = useQuery<PainterItem[], Error>({
    queryKey: [configType],
    queryFn: fetchPainters,
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      showError('Erro ao excluir. Verifique se não há pinturas associadas.');
    },
  });

  const handleEdit = (item: PainterItem) => {
  ...
  return (
    <Card className="backdrop-blur-sm bg-white/70 dark:bg-gray-800/70 shadow-xl border border-white/30 dark:border-gray-700/50">
      <CardHeader>
        <CardTitle className="flex items-center"><PaintBucket className="w-5 h-5 mr-2" /> Gerenciar Pintores</CardTitle>
      </CardHeader>
      <CardContent>
