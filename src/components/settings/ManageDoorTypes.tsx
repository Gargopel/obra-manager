...
import { Loader2, Plus, Trash2, Edit, Save, X, DoorClosed } from 'lucide-react';
import { showSuccess, showError } from '@/utils/toast';

interface DoorTypeItem {
  id: string;
  name: string;
}

const fetchDoorTypes = async (): Promise<DoorTypeItem[]> => {
  const { data, error } = await supabase.from('door_types').select('id, name').order('name');
  if (error) throw error;
  return data as DoorTypeItem[];
};

const ManageDoorTypes: React.FC = () => {
  const queryClient = useQueryClient();
  const [newItemName, setNewItemName] = useState('');
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingName, setEditingName] = useState('');

  const configType = 'door_types';
  const itemName = 'Tipo de Porta';

  const { data: items, isLoading, error } = useQuery<DoorTypeItem[], Error>({
    queryKey: [configType],
    queryFn: fetchDoorTypes,
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      showError('Erro ao excluir. Verifique se não há portas associadas.');
    },
  });

  const handleEdit = (item: DoorTypeItem) => {
  ...
  return (
    <Card className="backdrop-blur-sm bg-white/70 dark:bg-gray-800/70 shadow-xl border border-white/30 dark:border-gray-700/50">
      <CardHeader>
        <CardTitle className="flex items-center"><DoorClosed className="w-5 h-5 mr-2" /> Gerenciar Tipos de Portas</CardTitle>
      </CardHeader>
      <CardContent>
...
export default ManageDoorTypes;