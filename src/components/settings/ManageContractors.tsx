...
import { Loader2, Plus, Trash2, Edit, Save, X, HardHat } from 'lucide-react';
...
const ManageContractors: React.FC = () => {
  const queryClient = useQueryClient();
  const [newItemName, setNewItemName] = useState('');
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingName, setEditingName] = useState('');

  const configType = 'contractors';
  const itemName = 'Empreiteiro';

  const { data: items, isLoading, error } = useQuery<ContractorItem[], Error>({
    queryKey: [configType],
    queryFn: async () => {
      const { data, error } = await supabase.from(configType).select('id, name').order('name');
      if (error) throw error;
      return data as ContractorItem[];
    },
  });
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => showError('Erro ao criar: ' + error.message),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => showError('Erro ao excluir. Verifique se não há demandas associadas.'),
  });

  const handleEdit = (item: ContractorItem) => {
  ...
  return (
    <Card className="backdrop-blur-sm bg-white/70 dark:bg-gray-800/70 shadow-xl border border-white/30 dark:border-gray-700/50">
      <CardHeader>
        <CardTitle className="flex items-center"><HardHat className="w-5 h-5 mr-2" /> Gerenciar Empreiteiros</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex space-x-2 mb-6">
          <Input
            placeholder="Nome do novo empreiteiro"
            value={newItemName}
            onChange={(e) => setNewItemName(e.target.value)}
            className="flex-1"
          />
          <Button onClick={() => createMutation.mutate(newItemName.trim())} disabled={!newItemName.trim() || createMutation.isPending}>
        ...
        <div className="rounded-md border">
          <Table>
            <TableHeader>
              <TableRow><TableHead>Nome</TableHead><TableHead className="w-[150px] text-right">Ações</TableHead></TableRow>
            </TableHeader>
            <TableBody>
              {items?.map((item) => (
                <TableRow key={item.id}>
                  <TableCell className="font-medium">
                    {editingId === item.id ? (
                      <Input value={editingName} onChange={(e) => setEditingName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSave(item.id)} />
                    ) : item.name}
                  </TableCell>
                  <TableCell className="text-right space-x-2">
                    {editingId === item.id ? (
                      <>
                        <Button variant="ghost" size="icon" onClick={() => handleSave(item.id)} disabled={updateMutation.isPending}><Save className="w-4 h-4 text-green-600" /></Button>
                      ...
                      <>
                        ...
                        <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(item.id)} disabled={deleteMutation.isPending}><Trash2 className="w-4 h-4 text-destructive" /></Button>
                      </>
                    )}
                  </TableCell>
                </TableRow>
