...
const CreateDoorDialog: React.FC<CreateDoorDialogProps> = ({ open, onOpenChange }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => { if(!val) resetForm(); onOpenChange(val); }}>
      <DialogContent className="w-[95vw] max-w-lg backdrop-blur-md bg-white/90 dark:bg-gray-900/90 shadow-2xl border border-white/20 flex flex-col max-h-[90vh] p-4 md:p-6">
        <DialogHeader className="pb-2"><DialogTitle>Registrar Portas (Lote)</DialogTitle></DialogHeader>
            ...
            <div className="space-y-1.5">
              ...
              <div className="p-2 border rounded-md bg-background/50 max-h-24 overflow-y-auto">
                <ToggleGroup type="multiple" variant="outline" className="justify-start flex-wrap gap-1.5" value={selectedBlocks} onValueChange={setSelectedBlocks}>
                  {BLOCKS.map(b => <ToggleGroupItem key={b} value={b} className="w-8 h-8 text-xs data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">{b}</ToggleGroupItem>)}
                </ToggleGroup>
              </div>
            </div>

            <div className="space-y-1">
              <Label className="text-xs font-bold">Tipo/Local da Porta *</Label>
              <Select value={doorTypeId} onValueChange={setDoorTypeId} disabled={isLoadingConfig}>
                ...
                <SelectContent>{configData?.doorTypes.map(type => <SelectItem key={type.id} value={type.id} className="text-xs">{type.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        <DialogFooter className="pt-3 border-t mt-2">
          <Button onClick={() => createDoorMutation.mutate()} disabled={createDoorMutation.isPending || !isFormValid} className="w-full h-11 text-base">
